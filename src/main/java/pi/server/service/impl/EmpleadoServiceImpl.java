...
@WebServlet("pi/EmpleadoService")
public class EmpleadoServiceImpl extends HessianServlet implements EmpleadoService {

	public static String table = Empleado.class.getName();

	@Override
	public void cambiarClave(String app, int empleadoId, String claveActual, String claveNueva, String claveNuevaRep)
			throws Exception {
	...
	public List<Empleado> list(String app, int empresaId) {
		String[] req = {
				"persona", "sucursal", "cargo", "cargo.area", "empresa", "empresa.direccion",
				"empresa.direccion.persona"
		};
		List<Empleado> list = new ArrayList<>();
		try {
			list = CRUD.list(app, Empleado.class, req, "where a.empresa = " + empresaId + " order by b.apellidos asc");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	public List<Empleado> listUsuariosSistema(String app, int empresaId) throws Exception {
		...
		return CRUD.list(app, Empleado.class, req,
				"where a.empresa = " + empresaId + " and a.es_usuario_sistema is true order by b.apellidos asc");
	}
	...
	public List<Empleado> listVendedores(String app, int empresaId) {
		List<Empleado> list = new ArrayList<>();
		List<CargoPermiso> cargosPermisos;
		try {
			cargosPermisos = CRUD.list(app, CargoPermiso.class, "where permiso = " + Util.PER_VENDEDOR);
			...
			if (!cargosPermisos.isEmpty()) {
				...
				String where = " where a.empresa = " + empresaId + " and a.cargo in(" + sb.toString()
						+ ") and a.activo is true order  by b.apellidos asc";
				list = CRUD.list(app, Empleado.class, req, where);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
